...
export class State extends pulumi.ComponentResource {
  ...
  constructor(params: IComponentParams, private monitoring: Monitoring) {
  ...
  public storeInVault(
    name: string,
    key: string,
    value: pulumi.Input<string> | string,
  ...
  private keyvault(rg: azure.core.ResourceGroup) {
    ...
    identities.forEach((element) => {
      if (element.appId) {
        this.kvAccessPolicies.push(
          new azure.keyvault.AccessPolicy(
            element.name + "app",
            {
    ...
    const dataProtectionKey = new azure.keyvault.Key(
      "dataprotectionkey",
      {
        keyOpts: [
          "decrypt",
    ...
    const identitySigningCertificate = new azure.keyvault.Certificate(
      "identityCert",
      {
        certificatePolicy: {
          issuerParameters: {
          ...
          x509CertificateProperties: {
            ...
            subjectAlternativeNames: {
              dnsNames: ["localhost", `${stack}.identity.amphoradata.com`],
            },
            validityInMonths: 12,
          },
        },
  ...
  private createEventHubs(rg: azure.core.ResourceGroup) {
    ...
    const ehAuthRule = new azure.eventhub.AuthorizationRule(
      "ehAuthRule",
      {
    ...
    this.storeInVault(
      "TsiEventHubConnectionString",
      "TsiEventHub--ConnectionString",
  ...
  private createCosmosDb(rg: azure.core.ResourceGroup) {
    this.cosmosDb = new azure.cosmosdb.Account(
      "cosmosDb",
      {
        consistencyPolicy: {
    ...
    const dbA = new azure.cosmosdb.SqlDatabase(
      "cosmosSql_A",
      {
    ...
    this.storeInVault(
      "cosmosAccountPrimaryKey",
      "Cosmos--PrimaryKey",
      this.cosmosDb.primaryMasterKey
    );
    this.storeInVault(
      "cosmosAccountSecondaryKey",
      "Cosmos--SecondaryKey",
      this.cosmosDb.secondaryMasterKey
    );
    this.storeInVault(
      "cosmosAccountPrimaryReadonlyKey",
      "Cosmos--PrimaryReadonlyKey",
      this.cosmosDb.primaryReadonlyMasterKey
    );
    this.storeInVault(
      "cosmosAccountSecondaryReadonlyKey",
      "Cosmos--SecondaryReadonlyKey",
