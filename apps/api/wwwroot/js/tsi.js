...
async function tsi(id, signals, filters) {
  ...
  var renderLineGraph = async (from, to, timezone, bucketSize) => {
    ...
    var linechartTsqExpressions = await getLineChartExpressions(
      tsiClient,
      id,
      signals,
      filters,
    ...
    var result = await tsiClient.server.getTsqResults(
      "token",
      url,
    ...
    var data = tsiClient.ux.transformTsqResultsForVisualization(
      result,
  ...
  availability.render(
    [{ availabilityCount: { "": {} } }],
    {
      ...
      theme: "light",
      color: "red",
      brushMoveEndAction: renderLineGraph,
      offset: "Local",
      isCompact: true,
    },
    {
...
async function getLineChartExpressions(
  ...
  scheme
    .from_hue(Math.max(signals.length, 10))
    .scheme("tetrade")
    .variation("pastel");
